...
- [功能代码实现](#功能代码实现)
  - [FlHorizontalBarChart 组件实现](#flhorizontalbarchart-组件实现)
  - [主页面集成](#主页面集成)
...
1. **依赖管理问题**
   ...
   - 解决方案：在 pubspec.yaml 中指定合适的 fl_chart 版本，如 ^0.64.0，并执行 `flutter pub get` 命令更新依赖

2. **数据格式问题**
   - 问题：柱状图数据中的 value 字段类型需要为 double，直接使用 int 类型可能导致类型错误
   - 解决方案：在构建 BarChartRodData 时，对 int 类型的 value 进行类型转换，如 `(value as int).toDouble()`
